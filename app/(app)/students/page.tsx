...
import { STUDENTS, SUBSCRIPTIONS, WALLET_TRANSACTIONS } from "@/lib/mock-data";
...
function SubscriptionDetailRow({ studentId }: { studentId: string }) {
  const subs = SUBSCRIPTIONS.filter((s) => s.studentId === studentId);
  const txs = WALLET_TRANSACTIONS.filter((tx) => tx.studentId === studentId)
...
export default function StudentsPage() {
  ...
  return (
    <div className="space-y-6">
      <PageHeader
        title="Students"
        description="Student directory — click a row to inspect subscriptions and credits."
      />
          ...
          {filtered.map((s) => {
            ...
            return (
              <Fragment key={s.id}>
                ...
                {isExpanded && (
                  <SubscriptionDetailRow studentId={s.id} />
                )}
              </Fragment>
            );
          })}
