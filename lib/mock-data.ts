...
export interface MockSubscription {
  ...
  /**
   * Stripe refund metadata (new in 00072). Populated only when an admin
   * issues a Stripe-side refund via `issueStripeRefundAction`. Manual
   * BPM-only refunds (cash / Revolut / pay-at-reception) leave these
   * null because Stripe is never called.
   *
   *   * `stripeRefundId`        — id of the most recent Stripe refund
   *                                (re_...). Full per-attempt history
   *                                lives in op_finance_audit_log.
   *   * `refundedAmountCents`   — cumulative refunded amount, supports
   *                                partial refunds.
   *   * `refundStatus`          — Stripe-side refund state when the
   *                                action returned ("succeeded" |
   *                                "pending" | "failed").
   */
...
export interface MockEventPurchase {
  ...
  /**
   * Stripe refund metadata (new in 00072). Populated only when an admin
   * issues a Stripe-side refund via `issueStripeRefundAction`. See the
   * matching block on `MockSubscription` for semantics — including
   * support for partial refunds via `refundedAmountCents`.
   */
  stripeRefundId: string | null;
  refundedAmountCents: number;
  refundStatus: "succeeded" | "pending" | "failed" | null;
